...
class ConclusionAdapter(private val context: Context) : RecyclerView.Adapter<ConclusionViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ConclusionViewHolder {
        return ConclusionViewHolder(parent)
    }
    ...
    override fun onBindViewHolder(holder: ConclusionViewHolder, position: Int) {
        ...
        itemView.btnCard.setOnClickListener { listener!!.onClickConclusion(model) }
    }
    ...
    interface OnClickListListener { fun onClickConclusion(model: Conclusion) }
}
